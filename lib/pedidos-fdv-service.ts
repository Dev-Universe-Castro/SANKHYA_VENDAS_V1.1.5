...
export const pedidosFDVService = {
  ...
  async registrarPedido(pedido: PedidoFDV): Promise<number> {
    try {
      ...
      // Converter erro para JSON se for um objeto
      let erroString = null;
      if (pedido.ERRO) {
        if (typeof pedido.ERRO === 'object') {
          erroString = JSON.stringify(pedido.ERRO);
        } else {
          erroString = pedido.ERRO;
        }
      }
  ...
  async listarPedidosFDV(idEmpresa: number, filtros?: {
    ...
    try {
      ...
      const pedidos = result.map(row => {
        ...
        // Parse do JSON do erro (similar ao CORPO_JSON)
        let erroJson: any = undefined;
        if (row.ERRO) {
          if (typeof row.ERRO === 'string') {
            try {
              erroJson = JSON.parse(row.ERRO);
            } catch (e) {
              // Se falhar no parse, manter como string
              erroJson = row.ERRO;
            }
          } else {
            erroJson = row.ERRO;
